/*
 * ESP32-CAM Vehicle Number Plate Recognition (Modified for Serial Monitor)
 * 
 * Overview:
 * This modified project uses the ESP32-CAM to capture an image of a vehicle's number plate 
 * and sends it to a server for recognition. The recognized number plate is displayed on
 * the Serial Monitor of the Arduino IDE and sent to the server.
 *
 * Components Required:
 * - ESP32-CAM Module
 * - ESP32-CAM MB Module (for USB to UART communication)
 *
 * Note:
 * Ensure that the ESP32-CAM is correctly wired to the ESP32-CAM MB module.
 */
...
// WiFi credentials
const char* ssid = "Don";         // Replace "xxx" with your WiFi SSID
const char* password = "ankur123";      // Replace "xxx" with your WiFi Password
...
String apiKey = "Um3KIemnlaw3";             // Replace "xxx" with your API key
...
void setup() {
  ...
  Serial.println("System Initialized. Capturing and sending images...");
}

void loop() {
  // Automatically capture and send photo
  ...
  delay(5000);  // Wait 5 seconds before capturing again
}
...
int sendPhoto() {
  ...
  if (client.connect(serverName.c_str(), serverPort)) {
    ...
    Serial.print("Recognized Number Plate: ");
